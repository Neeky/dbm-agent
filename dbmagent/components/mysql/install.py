...
from dbmagent.components.mysql.config import MySQLSRConfig, MySQLSystemdConfig
from dbmagent.components.mysql.commons import get_mysql_version
from dbmagent.components.mysql.commons import export_cmds_to_path
from dbmagent.components.mysql.commons import pkg_to_basedir, default_pkg
from dbmagent.components.mysql.commons import (
    export_header_files,
    export_so_files,
    create_os_user_for_mysql,
    create_mysql_dirs,
)
from dbmagent.components.mysql.exceptions import MySQLSystemdFileNotExists
from dbmagent.components.mysql.exceptions import MySQLPkgFileNotExistsException
from dbmagent.components.mysql.exceptions import InstanceHasBeenInstalledException
from dbmagent.components.mysql.asserts import (
...
def create_init_sql_file(version: str = None):
    ...
    if version.startswith("8.0"):
        sql_file = Path(dbmagent.__file__).parent / "static/cnfs/init-8.0.x.sql"
    elif version.startswith("5.7"):
        sql_file = Path(dbmagent.__file__).parent / "static/cnfs/init-5.7.x.sql"
    else:
        message = (
